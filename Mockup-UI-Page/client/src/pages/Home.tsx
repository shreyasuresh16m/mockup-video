    ...
    >
      ...
      <AnimatePresence mode="wait" initial={false}>
        {isActive ? (
          ...
          >
            {/* Back button — shown for steps 2+ */}
            {onBack && (
              <button
                onClick={onBack}
                className="mb-6 flex items-center gap-1.5 text-[0.82em] text-white/40 hover:text-white/80 transition-colors"
              >
...
const Chip: React.FC<{ label: string; selected: boolean; onClick: () => void }> = ({
  label,
  selected,
  onClick,
}) => (
  <motion.button
    ...
    whileTap={{ scale: 0.98 }}
    className={cn(
      "px-6 py-3 rounded-full border transition-all duration-200 text-[0.9em] font-medium",
      selected
      ...
      {components.map((comp) => {
        ...
        return (
          ...
          >
            ...
            {isSelected && placed && (
              <span className="text-[0.7em] bg-white/10 text-white/60 px-2 py-0.5 rounded-full shrink-0">
                Repin
              </span>
            )}
          </motion.button>
...
function getAISuggestedPosition(
  ...
  // Component-aware spatial intelligence
  ...
  if (name.includes("button") || name.includes("control") || name.includes("keypad"))
    return { x: 74 + seed, y: 46 + seed * 2 };
  ...
  // Smart grid fallback for unknown components
...
export default function Home({ projectName, initialOfferings, onLogout, onOfferingsChange, onBuildBrochure, onBackToDashboard }: HomeProps) {
  ...
  // Pin state: one pin per component
  const [pinMap, setPinMap] = useState<PinMap>({});
  const [activeComponent, setActiveComponent] = useState<string>("");

  const [imageScale, setImageScale] = useState(1);
  const [videoConfig, setVideoConfig] = useState({ option: "1", speed: "1x", quality: "1080p" });

  // Offerings — initialised from persisted state so returning to this project restores them
  const [savedOfferings, setSavedOfferings] = useState<Offering[]>(initialOfferings);

  // AI pin placement tracking
  ...
  const resetWorkflow = () => {
    ...
    setRegenSeed(0);
    setImageScale(1);
    setVideoConfig({ option: "1", speed: "1x", quality: "1080p" });
  ...
  const handleImageClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Mark as user-adjusted (no longer AI-suggested)
  ...
  // When entering step 3 — AI auto-place all components with a staggered animation effect
  useEffect(() => {
    if (activeStep === 3 && components.length > 0) {
      ...
      // Stagger pin appearance for a satisfying AI-placing effect
      components.forEach((comp, i) => {
        setTimeout(() => {
  ...
  // When components change, reset pins for removed ones
  useEffect(() => {
    setPinMap((prev) => {
      const next: PinMap = {};
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background overflow-x-hidden">
      ...
      {/* ── Main ── */}
      <main className="flex-1 w-full flex justify-center py-32 px-4 relative">
        {/* Backdrop panel */}
          ...
          <StepContainer
            ...
            readOnlyContent={
              <div className="flex items-center gap-3 text-[0.9em]">
                ...
                <div>
                  <p className="text-white font-medium">Media uploaded</p>
                  <p className="text-[0.8em] text-white/40">Hi-res · 4:3 aspect ratio</p>
                </div>
              </div>
            }
          >
            <div className="space-y-7">
              ...
              >
                <div
                  className={cn(
                    "w-18 h-18 rounded-full flex items-center justify-center mb-5 transition-transform group-hover:scale-105",
                    isUploaded ? "bg-[#00A3E0]/20" : "bg-white/10"
                  )}
                >
                  {isUploaded ? (
                    <CheckCircle2 className="w-9 h-9 text-[#00A3E0]" />
                  ) : (
                    <UploadCloud className="w-9 h-9 text-white" />
                  )}
                </div>
                <p className="text-[1em] font-medium mb-1">
                  {isUploaded ? "File selected" : "Drag & drop or click to browse"}
                </p>
                <p className="text-white/45 text-[0.8em] mb-5">
                  {isUploaded ? "Ready to continue" : "Supported: .jpg  .jpeg  .png  .mpeg"}
                </p>
              ...
              <ActionRow onContinue={() => handleContinue(2)} continueDisabled={!isUploaded} />
            </div>
          </StepContainer>
            ...
            readOnlyContent={
              <div className="text-[0.9em] space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {components.map((c) => (
                      <span key={c} className="bg-[#00A3E0]/15 border border-[#00A3E0]/35 px-4 py-1.5 rounded-full text-white text-[0.9em]">{c}</span>
                    ))}
                  </div>
                </div>
              </div>
                ...
                {(showCarOptions || showLobbyOptions) && (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-3">
                      {showCarOptions && (
                        <>
                          <Chip label="COP" selected={components.includes("COP")} onClick={() => toggleArrayItem(components, setComponents, "COP")} />
                          <Chip label="Ceiling" selected={components.includes("Ceiling")} onClick={() => toggleArrayItem(components, setComponents, "Ceiling")} />
                        </>
                      )}
                      {showLobbyOptions && (
                        <>
                          <Chip label="LCI" selected={components.includes("LCI")} onClick={() => toggleArrayItem(components, setComponents, "LCI")} />
                          <Chip label="Door" selected={components.includes("Door")} onClick={() => toggleArrayItem(components, setComponents, "Door")} />
                        </>
                      )}
                    </div>
                  </motion.div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5">
                ...
                <div className="relative rounded-xl overflow-hidden border border-white/20 bg-black h-[400px]">
                  ...
                  >
                    <img
                      src="https://images.unsplash.com/photo-1579762593175-20226054cad0?w=1200&h=900&fit=crop"
          ...
          >
            <div className="space-y-5">
              ...
              <div className="relative rounded-xl overflow-hidden border border-white/20 bg-black h-[520px]">
                ...
                {/* Render one callout per placed component */}
                {components.map((comp, idx) => {
                  ...
                  // Line extends 80px horizontally, 30px vertically upward
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <p className="text-[0.65em] text-[#00A3E0] font-semibold uppercase tracking-wider mb-0.5">
                          {comp}
                        </p>
                        <p className="text-[0.72em] text-white/60">Generated component</p>
                      </div>

                      {/* Small bounding highlight around pin */}
          ...
          <StepContainer
            ...
            readOnlyContent={
              <div className="flex items-center gap-3 text-[0.85em]">
                ...
                <div>
                  <p className="text-white font-medium">Video {videoConfig.option}</p>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <DownloadTile
                  ...
                  label="Video"
                  sub={`${videoConfig.quality} · ${videoConfig.speed} speed`}
                />
                <DownloadTile
                  icon={<Download className="w-8 h-8 text-white/80" />}
                  label="HD Component Images"
                  sub={`${components.length || 0} component${components.length !== 1 ? "s" : ""} · Full resolution`}
                />
              </div>

              {/* Per-component HD downloads */}
              {components.length > 0 && (
                <div className="bg-[#252525] rounded-2xl border border-white/10 p-6 space-y-4">
                  <p className="text-[0.8em] text-white/50 uppercase tracking-wider font-medium">
                    Individual HD Component Files
                  </p>
                  <div className="divide-y divide-white/5">
                    {components.map((comp) => (
                      <div key={comp} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center">
                            <ImageIcon className="w-4 h-4 text-white/40" />
                          </div>
                          <div>
                            <p className="text-[0.9em] font-medium text-white">{comp}</p>
                            <p className="text-[0.75em] text-white/40">PNG · 4K · Transparent background</p>
                          </div>
                        </div>
                        <button className="flex items-center gap-1.5 text-[0.8em] text-white/60 hover:text-white px-4 py-2 rounded-full border border-white/10 hover:border-white/30 hover:bg-white/5 transition-all">
              ...
              <div className="border-t border-white/10 pt-6 space-y-4">
                ...
                {/* Check if current config is already saved */}
                {(() => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-2.5 px-6 py-3 rounded-xl font-medium text-[0.9em] transition-all border",
                        alreadySaved || components.length === 0
                          ? "border-white/8 text-white/25 cursor-not-allowed bg-transparent"
                          : "border-[#1450f5]/40 text-[#7b9fff] hover:bg-[#1450f5]/10 hover:border-[#1450f5]/60 active:scale-95"
                      )}
                    >
                      <BookmarkPlus className="w-4 h-4" />
                      {alreadySaved
                        ? "Configuration already saved"
                        : `Save as Offering ${savedOfferings.length + 1}`}
                    </button>
                  );
                })()}
                ...
                {savedOfferings.length > 0 && (
                  <button
                    ...
                    className="flex items-center gap-2 text-[0.82em] text-white/35 hover:text-white/60 transition-colors"
                  >
                    <RotateCcw className="w-3.5 h-3.5" />
                    Add another configuration (Offering {savedOfferings.length + 1})
                  </button>
                )}
              </div>
            </div>
